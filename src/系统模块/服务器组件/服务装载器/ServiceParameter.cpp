...
void CServiceParameter::OnOK()
{
	...
	//高级权限
	//税收
	OptionParameter.GameServiceOption.cbTaxRate=GetDlgItemInt(IDC_EDIT_RATIO,NULL,FALSE);

	//是否为防作弊
//	OptionParameter.GameServiceOption.cbFZB=(((CButton *)GetDlgItem(IDC_CONTROL_FZB))->GetCheck()==BST_CHECKED)?1:0;
	...
	//最高积分
	OptionParameter.GameServiceOption.lHighScore=GetDlgItemInt(IDC_EDIT_HighScore,NULL,FALSE);

	//最低积分
	OptionParameter.GameServiceOption.lLessScore=GetDlgItemInt(IDC_EDIT_LessScore,NULL,FALSE);

	//FZB
	OptionParameter.GameServiceOption.cbFZB=GetDlgItemInt(IDC_EDIT_FZB,NULL,FALSE);

	//限制局数
	OptionParameter.GameServiceOption.cbLimitDraw=(((CButton *)GetDlgItem(IDC_LIMIT_DRAW))->GetCheck()==BST_CHECKED)?TRUE:FALSE;
...
void CServiceParameter::UpdateOptionParameter()
{
	...
	//游戏类型
	pComboBox->Clear();
	for (i=0;i<CountArray(wGameGenre);i++)
	{
	...
	//最高积分
	if (m_OptionParameter.GameServiceOption.lHighScore==0) SetDlgItemText(IDC_EDIT_HighScore,TEXT(""));
	else SetDlgItemInt(IDC_EDIT_HighScore,m_OptionParameter.GameServiceOption.lHighScore,FALSE);

	//最低积分
	if (m_OptionParameter.GameServiceOption.lLessScore==0) SetDlgItemText(IDC_EDIT_LessScore,TEXT(""));
	else SetDlgItemInt(IDC_EDIT_LessScore,m_OptionParameter.GameServiceOption.lLessScore,FALSE);

	//最低积分
	if (m_OptionParameter.GameServiceOption.cbFZB==0) SetDlgItemText(IDC_EDIT_FZB,TEXT(""));
	else SetDlgItemInt(IDC_EDIT_FZB,m_OptionParameter.GameServiceOption.cbFZB,FALSE);


	//限制局数
	BYTE cbLimitDraw=m_OptionParameter.GameServiceOption.cbLimitDraw;
	((CButton *)GetDlgItem(IDC_LIMIT_DRAW))->SetCheck((cbLimitDraw==TRUE)?BST_CHECKED:BST_UNCHECKED);
	...
	//附加属性
//税收
	if (m_OptionParameter.GameServiceOption.cbTaxRate==0) SetDlgItemText(IDC_EDIT_RATIO,TEXT("0"));
	else SetDlgItemInt(IDC_EDIT_RATIO,m_OptionParameter.GameServiceOption.cbTaxRate,FALSE);

	//是否为防作弊房间
//	BYTE cbFZB=m_OptionParameter.GameServiceOption.cbFZB;
//	((CButton *)GetDlgItem(IDC_CONTROL_FZB))->SetCheck((cbFZB==TRUE)?BST_CHECKED:BST_UNCHECKED);
