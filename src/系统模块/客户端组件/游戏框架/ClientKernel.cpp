...
extern CFlyGameSound  g_myCFlyGameSound;
...
//获得播放声音
CString CClientKernel::GetPlaySoundFileStr(CString inStr, CString *charStr)
{
CString resultStr;
CString findStr="PlaySound:";

if ( inStr.Find(findStr) >= 0 ) 
{
	int findInt =inStr.Find(";") ;
	if (findInt > 0)
	{
resultStr =  inStr.Mid(10, findInt-10);
*charStr = inStr.Mid(findInt+1);
	}
}
return resultStr;	
}
	...
	//处理动态语音
bool CClientKernel::DealwithDTSoundMsg(CMD_GF_UserChat * pNormalTalk)
{
	CString strFile,strTemp;
	CTime tmCur = CTime::GetCurrentTime();
	CString strTime = tmCur.Format("%m%d");
	strFile.Format("log\\%sDealwithDTSoundMsg.log",strTime);

	strTemp.Format("into DealwithDTSoundMsg");
	WriteLog(strFile, strTemp);
bool resultBool = false;

	CString chatStr;
CString mp3FileStr = GetPlaySoundFileStr( pNormalTalk->szChatMessage , &chatStr);
int sendInt = chatStr.GetLength();
if ( sendInt> 0 )
{
	memset((char *)pNormalTalk->szChatMessage,0,sizeof(pNormalTalk->szChatMessage));
memcpy( (char *)pNormalTalk->szChatMessage, (char *) chatStr.GetBuffer(sendInt), sendInt);
resultBool = true;
}
...
{
CString showMsg;
CString sendUserName;
showMsg.Format("MP3:%s        Text:%s", mp3FileStr, chatStr);
//AfxMessageBox(showMsg);
//			if (pSendUserItem!=NULL) 
			{
	//		sendUserName.Format("%s", pSendUserItem->GameUserInfo.szName);
			}
	//显示消息
//m_MessageHandle.InsertUserTalk( sendUserName.GetBuffer(sendUserName.GetLength()),NULL, chatStr.GetBuffer(chatStr.GetLength()),RGB( 255, 0, 0),false);

	//播放语音
	if ( mp3FileStr.GetLength() > 0 )
	{
		mp3FileStr.Format("DDZ\\sound\\scenesound\\%s", mp3FileStr);
	...
	case IPC_SUB_SOCKET_RECV:	//数据接收
		{
			...
			//处理动态语音
			CString inStr;
//			inStr.Format("%s", pNormalTalk->szMessage);
			if ( DealwithDTSoundMsg(   pUserChat))
			{}
			else
			{
				return true;
	...
	case IPC_SUB_SERVER_INFO:	//房间信息
		{
			...
			_snprintf(szTitle,sizeof(szTitle),TEXT("%s ［ %s - 第 %d 桌 ］"),m_ServerAttribute.szKindName,m_ServerAttribute.szServerName,m_wTableID+1);
