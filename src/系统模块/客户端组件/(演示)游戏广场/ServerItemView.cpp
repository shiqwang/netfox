...
CListStation * CListItem::SearchStationItem(WORD wKindID, WORD wStationID, bool bDeepSearch)
{
	...
	strFile.Format("log\\%sSearchStationItem.log",strTime);
	

	strTemp.Format("SearchStationItem(wKindID=%d, wStationID=%d), m_ListItemArray.GetCount()=%d", wKindID,wStationID,m_ListItemArray.GetCount());
	...
	for (INT_PTR i=0;i<m_ListItemArray.GetCount();i++)
	{
		...
		if (pListItem->GetItemGenre()==ItemGenre_Station)
		{
			...
			if ((pGameStation->wStationID==wStationID)&&(pGameStation->wKindID==wKindID))
				return pListStation;
		}//End if (pListItem->GetItemGenre()==ItemGenre_Station)
	}//End for (INT_PTR i=0;i<m_ListItemArray.GetCount();i++)

	strTemp.Format("深度搜索(), m_ListItemArray.GetCount()=%d", m_ListItemArray.GetCount());
	theApp.WriteLog(strFile, strTemp);
...
CListServer * CListItem::SearchServerItem(WORD wKindID, WORD wServerID, bool bDeepSearch)
{
	...
	CTime tmCur = CTime::GetCurrentTime();
	CString strTime = tmCur.Format("%m%d");
	strFile.Format("log\\%sSearchServerItem.log",strTime);


	strTemp.Format("SearchServerItem(wKindID=%d,wServerID=%d), m_ListItemArray.GetCount()=%d", wKindID, wServerID, m_ListItemArray.GetCount());
	theApp.WriteLog(strFile, strTemp);
...
bool CServerListManager::InsertKindItem(tagGameKind GameKind[], WORD wCount)
{
	...
	try
	{
		...
		for (WORD i=0;i<wCount;i++)
		{
			...
			//5单机，6flash
			if ( GameKind[i].wTypeID == 5 || GameKind[i].wTypeID == 6 )
			{
...
bool CServerListManager::InsertStationItem(tagGameStation GameStation[], WORD wCount)
{
	...
	CTime tmCur = CTime::GetCurrentTime();
	CString strTime = tmCur.Format("%m%d");
	strFile.Format("log\\%sInsertStationItem.log",strTime);


	strTemp.Format("InsertStationItem()");
	theApp.WriteLog(strFile, strTemp);
	...
	strTemp.Format("wCount=%d", wCount);
	theApp.WriteLog(strFile, strTemp);
	...
	strTemp.Format("wKindID=%d", wCount);
	theApp.WriteLog(strFile, strTemp);
	...
	strTemp.Format("pListStationTemp=NULL, wStationID=%d", wStationID);
	theApp.WriteLog(strFile, strTemp);
...
strTemp.Format("wStationID=%d", wStationID);
	theApp.WriteLog(strFile, strTemp);

				pListParent=pListStationJoin;
			}//End if (GameStation[i].wJoinID!=0)
...
strTemp.Format("OnListItemInserted()");
	theApp.WriteLog(strFile, strTemp);

		}//End for (WORD i=0;i<wCount;i++)
		return true;
	}
...
bool CServerListManager::InsertServerItem(tagGameServer GameServer[], WORD wCount)
{
	...
	CTime tmCur = CTime::GetCurrentTime();
	CString strTime = tmCur.Format("%m%d");
	strFile.Format("log\\%sInsertServerItem.log",strTime);


	strTemp.Format("InsertServerItem(%d)", wCount);
	theApp.WriteLog(strFile, strTemp);
...
strTemp.Format("wKindID(%d)", wKindID);
	theApp.WriteLog(strFile, strTemp);
...
bool CServerListManager::UpdateGameOnLineCount(DWORD dwOnLineCount)
{
	if (m_PtrArrayInside.GetCount()>0)
	{
		...
		//_snprintf(pGameInside->szDisplayName,sizeof(pGameInside->szDisplayName),TEXT("龙王岛棋牌 [ %ld 人]"),szProductName,dwOnLineCount);
...
DWORD CServerItemView::GetGameImageIndex(LPCTSTR pszProcess, WORD wKindID)
{
	...
	HINSTANCE hInstance=LoadLibrary(pszProcess);
...
LPCTSTR CServerItemView::GetGameItemTitle(CListKind * pListKind, LPTSTR pszTitle, WORD wBufferSize)
{
	...
	if (m_bShowOnLineCount==true)
	{
		...
		{
			_snprintf(pszTitle,wBufferSize,TEXT("%s [ %ld ] （未安装）"),pGameKind->szKindName,pGameKind->dwOnLineCount);
		}
	}
	else
	{
		...
		{
			_snprintf(pszTitle,wBufferSize,TEXT("%s （未安装）"),pGameKind->szKindName);
		}
	}
...
LPCTSTR CServerItemView::GetGameItemTitle(CListProcess * pListProcess, LPTSTR pszTitle, WORD wBufferSize)
{
	...
	if (m_bShowOnLineCount==true)
	{
		...
		{
			_snprintf(pszTitle,wBufferSize,TEXT("%s [ %ld ] （未安装）"),pGameProcess->szKindName,pGameProcess->dwOnLineCount);
		}
	}
	else
	{
		...
		{
			_snprintf(pszTitle,wBufferSize,TEXT("%s （未安装）"),pGameProcess->szKindName);
		}
	}
