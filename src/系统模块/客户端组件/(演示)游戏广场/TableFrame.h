...
interface ITableFrameView : public IUnknownEx
{
	...
	//配置函数
	virtual bool __cdecl InitTableFrame(WORD wTableCount, WORD wPlayerCount, const tagGameKind * pGameKind, IUnknownEx * pIUnknownEx, int isFZBRoom)=NULL;
	//销毁函数
...
protected:
	...
	//加载资源
	bool LoadFromResource(HINSTANCE hInstance);
	//卸载资源
	void DestroyResource();
	//加载位图
	bool LoadImageFromResource(HINSTANCE hInstance, LPCTSTR pszResourceName);
};
...
public:
	...
	//配置函数
	virtual bool __cdecl InitTableFrame(WORD wTableCount, WORD wChairCount, const tagGameKind * pGameKind, IUnknownEx * pIUnknownEx, int isFZBRoom );
	//销毁函数
