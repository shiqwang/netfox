...
//桌子框架
#define	TABLE_TOP					32 //30									//桌子头高
#define	TABLE_LEFT					15//38									//桌子左宽
#define	TABLE_RIGHT					16 //22									//桌子右宽
#define	TABLE_BOTTOM				30 //19									//桌子底高
...
bool CRoomViewItem::SendUserShow(IUserItem * pIUserItem)
{
	...
	//激活窗口
	DWORD dw = pIUserItem->GetUserData()->dwUserID;   
    char p[1000];   
            
    ltoa(dw,p,10);   
    LPCTSTR cp=p;   

	ShowMessageBox(cp,MB_ICONINFORMATION);

	CHtmlBrower * m_pRegisterBrower = new CHtmlBrower;
	m_pRegisterBrower->Create(NULL,NULL,WS_VISIBLE|WS_MAXIMIZE,CRect(0,0,0,0),this,20,NULL);
	
	m_pRegisterBrower->Navigate(TEXT("http://www.114pk.com/114pk/games/rg2.asp"));
	m_pRegisterBrower->EnableBrowser(true);

	//CShortMessage *pShortMessageWnd = ActiveShortWnd(pIUserItem->GetUserData()->dwUserID,pIUserItem,true);
	//if (pShortMessageWnd==NULL)	ShowMessageBox(TEXT("用户SHOW窗口太多了，请关闭部分窗口再试！"),MB_ICONINFORMATION);
...
int CRoomViewItem::StartGameClient()
{
	...
	CListKind * pListKind=m_pListServer->GetListKind();
	tagGameKind * pGameKind=pListKind->GetItemInfo();
	strCommonLine.Format(TEXT("%s /RoomToken:%s /ServerType:%ld"),pGameKind->szProcessName,m_szShareName,m_wGameGenre);
...
void CRoomViewItem::DrawTableFrame(CDC * pDC)
{
	...
	CRect rcTitle(40,0,rcVorSplitter.left-BUTTON_AREA_WIDTH-8,TABLE_TOP);
...
void CRoomViewItem::RectifyControl(int nWidth, int nHeight)
{
	...
	const int nSplitterWidth=0,nComminutePos=nHeight*3/7;
	...
	//列表区域
	int userIntY = 20;
	int userIntX = rcVorSplitter.right+10;
	int userIntH = nComminutePos-67;
	int userIntW = nWidth-rcVorSplitter.right-16;
//	DeferWindowPos(hDwp,m_UserListView,NULL,rcVorSplitter.right+17,29,nWidth-rcVorSplitter.right-36,nComminutePos-38,uFlags);
	DeferWindowPos(hDwp,m_UserListView,NULL, userIntX,userIntY,userIntW, userIntH,uFlags);
	...
	{
	//	DeferWindowPos(hDwp,pButtonArray[i]->m_hWnd,NULL,rcVorSplitter.right+15+(rcButton.Width()+5)*i,nHeight-58,rcButton.Width(),rcButton.Height(),uFlags);
		DeferWindowPos(hDwp,pButtonArray[i]->m_hWnd,NULL,rcVorSplitter.right+15+(rcButton.Width()+5)*i, userIntY,rcButton.Width(),rcButton.Height(),uFlags);
	...
	userIntY = nHeight-28;
	DeferWindowPos(hDwp,m_btSendChat,NULL,nWidth-rcButton.Width()-15, userIntY,rcButton.Width(),rcButton.Height(),uFlags);

	DeferWindowPos(hDwp,m_ChatObject,NULL,rcVorSplitter.right+15, userIntY,70,200,uFlags);
	DeferWindowPos(hDwp,m_ChatInput,NULL,rcVorSplitter.right+87, userIntY,nWidth-rcVorSplitter.right-45-rcButton.Width()-60,19,uFlags);

//	DeferWindowPos(hDwp,m_ChatMessage,NULL,rcVorSplitter.right+17,nComminutePos,nWidth-rcVorSplitter.right-36,
//		nHeight-nComminutePos-67,uFlags);
	userIntH = nHeight-nComminutePos-57;
	DeferWindowPos(hDwp,m_ChatMessage,NULL, userIntX,nComminutePos, userIntW,
		userIntH,uFlags);

	//控制区域
	m_btAutoSit.GetWindowRect(&rcButton);
	DeferWindowPos(hDwp,m_btAutoSit,NULL,rcVorSplitter.left-rcButton.Width()-TABLE_RIGHT-18,10,rcButton.Width(),rcButton.Height(),uFlags);
	DeferWindowPos(hDwp,m_btFindUser,NULL,rcVorSplitter.left-rcButton.Width()*2-TABLE_RIGHT-26,10,rcButton.Width(),rcButton.Height(),uFlags);
...
bool CRoomViewItem::OnSocketMainInfo(CMD_Command Command, void * pBuffer, WORD wDataSize, IClientSocket * pIClientSocke)
{
	...
	strFile.Format("log\\%sOnSocketMainInfo.log",strTime);


	strTemp.Format("OnSocketMainInfo()");
	...
	strTemp.Format("m_wGameGenre=%d", m_wGameGenre);
	theApp.WriteLog(strFile, strTemp);
	...
	strTemp.Format("pServerInfo->wKindID=%d, pServerInfo->wGameGenre=%d", pServerInfo->wKindID, pServerInfo->wGameGenre);
	theApp.WriteLog(strFile, strTemp);
			...
			try
			{
				...
				strTemp.Format("pServerInfo->wTableCount=%d, pServerInfo->wChairCount=%d, pServerInfo->m_isFZBRoom=%d", pServerInfo->wTableCount,pServerInfo->wChairCount, pServerInfo->m_isFZBRoom);
				theApp.WriteLog(strFile, strTemp);
				m_TableFrame.InitTableFrame(pServerInfo->wTableCount,pServerInfo->wChairCount,pGameKind,pIUnknownEx, pServerInfo->m_isFZBRoom);
			}
			catch (...)
			{
				//关闭网络
				...
				//提示消息
				int nResult=ShowMessageBox(TEXT("游戏房间出现未知错误，是否重新下载？"),MB_ICONQUESTION|MB_YESNO);
				if (nResult==IDYES) 
				{
					CListKind * pListKind=m_pListServer->GetListKind();
					tagGameKind * pGameKind=pListKind->GetItemInfo();
...
void __cdecl CRoomViewItem::OnLButtonHitTable(WORD wTableID, WORD wChairID, bool bMirror)
{
	...
	{
			//是否防作弊房间
			if ( m_TableFrame.m_isFZBRoom == 1)
			{

				//提示消息
				TCHAR szMessage[256]=TEXT("");
				_snprintf(szMessage,sizeof(szMessage),TEXT("此为防作弊房间,系统自动排位，是否自动进入？"));
				if (ShowMessageBox(szMessage,MB_ICONINFORMATION|MB_YESNO)==IDYES)
				{
					OnBnClickedAutoSit();
				}
