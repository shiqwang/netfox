...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt">
    ...
    <xsl:sort select="@Name" order="ascending"/>
        <h2>
        <xsl:if test="@solution">解决方案: <xsl:value-of select="@solution"/></xsl:if>
        <xsl:if test="not(@solution)">项目: <xsl:value-of select="@name"/>
                ...
                <tr class="row">
                    ...
                    <td class="content">
                        <xsl:if test="count(event[@error-level='3'])=1">
                            ...
                            <xsl:if test="@description='Converted'">已转换</xsl:if>
                            <xsl:if test="@description!='Converted'"><xsl:value-of select="@description"/></xsl:if>
                            </xsl:for-each>
                        </xsl:if>
                        <xsl:if test="count(event[@error-level='3'])!=1 and count(event[@error-level='3' and @description='Converted'])!=0">已转换
                        </xsl:if>
                    </td>
            ...
            <tr valign="top">
                <td class="foot">
                    <xsl:if test="count(source)!=1">
                        <xsl:value-of select="count(source)"/> 文件
                    </xsl:if>
                    <xsl:if test="count(source)=1">
                        1 个文件
                    </xsl:if>
                ...
                <td class="foot">
                    已转换: <xsl:value-of select="count(source/event[@error-level='3' and @description='Converted'])"/><BR/>
                    未转换 <xsl:value-of select="count(source) - count(source/event[@error-level='3' and @description='Converted'])"/>
                </td>
    ...
    <xsl:template match="UpgradeLog">
        <html>
            ...
            <body topmargin="0" leftmargin="0" rightmargin="0" onclick="outliner();">
                ...
                <p><span class="note">
                <b>转换时间:</b>  <xsl:value-of select="Properties/Property[@Name='Date']/@Value"/>  <xsl:value-of select="Properties/Property[@Name='Time']/@Value"/><br/>
                </span></p>
