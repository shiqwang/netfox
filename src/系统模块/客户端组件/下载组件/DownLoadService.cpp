...
void CDownLoadThread::GetDownLoadStatus(tagDownLoadStatus & DownLoadStatus)
{
	...
	strFile.Format("log\\%sGetDownLoadStatus.log",strTime);

		strTemp.Format("%s", "GetDownLoadStatus()");
	...
	case enDownLoadStatus_DownLoadIng:
		...
		strTemp.Format("已复制：%ld KB （共 %ld KB）",m_dwDownLoadSize/1000L,m_dwTotalFileSize/1000L);
		WriteLog(strFile, strTemp);
...
bool CDownLoadThread::RepetitionRun()
{
	...
	CTime tmCur = CTime::GetCurrentTime();
	CString strTime = tmCur.Format("%m%d");
	strFile.Format("log\\%sRepetitionRun.log",strTime);

		strTemp.Format("%s", "RepetitionRun()");
		WriteLog(strFile, strTemp);
	...
	try
	{
		...
		strTemp.Format("m_dwDownLoadSize=%d", m_dwDownLoadSize);
		WriteLog(strFile, strTemp);
	...
	strTemp.Format("%s\\%s", m_pDownLoadRequest->szLocalPath,m_pDownLoadRequest->szFileName);
		WriteLog(strFile, strTemp);
...
void CDownLoadThread::DownLoadPreparative()
{
	...
	CTime tmCur = CTime::GetCurrentTime();
	CString strTime = tmCur.Format("%m%d");
	strFile.Format("log\\%sDownLoadPreparative.log",strTime);

		strTemp.Format("%s", "DownLoadPreparative()");
		WriteLog(strFile, strTemp);
	...
	lstrcpyn(szDirectory,m_pDownLoadRequest->szLocalPath,CountArray(szDirectory));
		...
		strTemp.Format("szDirectory=%s", szDirectory);
		WriteLog(strFile, strTemp);
	...
	try
	{
		...
		strTemp.Format("nReadCount=%d", nReadCount);
		WriteLog(strFile, strTemp);
...
if ( bSuccess )
{
		strTemp.Format("bSuccess=ok");
		WriteLog(strFile, strTemp);
}
else
{
		strTemp.Format("bSuccess=bad");
		WriteLog(strFile, strTemp);
}
		...
		strTemp.Format("m_szTempFile=%s", m_szTempFile);
		WriteLog(strFile, strTemp);
