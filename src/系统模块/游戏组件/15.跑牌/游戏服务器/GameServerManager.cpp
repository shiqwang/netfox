...
CGameServiceManager::CGameServiceManager(void)
{
	...
	lstrcpyn(m_GameServiceAttrib.szDataBaseName,TEXT("WHLandDB"),CountArray(m_GameServiceAttrib.szDataBaseName));
	lstrcpyn(m_GameServiceAttrib.szDescription,TEXT("跑牌游戏服务组件"),CountArray(m_GameServiceAttrib.szDescription));
	lstrcpyn(m_GameServiceAttrib.szClientModuleName,TEXT("Fast.EXE"),CountArray(m_GameServiceAttrib.szClientModuleName));
	lstrcpyn(m_GameServiceAttrib.szServerModuleName,TEXT("FastServer.DLL"),CountArray(m_GameServiceAttrib.szServerModuleName));
...
bool __cdecl CGameServiceManager::RectifyServiceOption(tagGameServiceOption * pGameServiceOption)
{
	...
	{
		pGameServiceOption->lLessScore=__max(pGameServiceOption->lCellScore*156L,pGameServiceOption->lLessScore);
