...
void CControlWnd::SetControlInfo(BYTE cbCenterCard, BYTE cbActionMask, tagGangCardResult & GangCardResult )
{
	...
	strTemp.Format("SetControlInfo cbCenterCard=%d, BYTE cbActionMask=%d ", cbCenterCard,  cbActionMask);
...
void CControlWnd::OnPaint()
{
	...
	for (BYTE i=0;i<CountArray(cbItemKind);i++)
	{
		if ((m_cbActionMask&cbItemKind[i])!=0)
		{
			...
			++cbCurrentItem;
			nYPos+=ITEM_HEIGHT;
		}//End if ((m_cbActionMask&cbItemKind[i])!=0)
	}//End for (BYTE i=0;i<CountArray(cbItemKind);i++)
	...
	}//End for (BYTE i=0;i<CountArray(m_cbGangCard);i++)
...
int CControlWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_btTingCard.Create(NULL,WS_CHILD|WS_VISIBLE|WS_DISABLED,rcCreate,this,IDC_TING_CARD);
...
//控制听牌按钮
//0=隐藏  , 1=显示
void CControlWnd::ShowHideTingCardButton(int opTypeInt )
{
	//听牌
	m_btTingCard.EnableWindow(opTypeInt);
}
//////////////////////////////////////////////////////////////////////////
