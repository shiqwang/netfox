...
bool CGameLogic::AnalyseCard(BYTE cbCardIndex[MAX_INDEX], tagWeaveItem WeaveItem[], BYTE cbWeaveCount, CAnalyseItemArray & AnalyseItemArray)
{
		...
		do
		{
			...
			if (bEnoughCard==true)
			{
				...
				if (cbCardEye!=0)
				{
					...
					}//End for (i=0;i<cbLessKindItem;i++) 
			...
			if (cbIndex[cbLessKindItem-1]==(cbKindItemCount-1))
			{
				...
				}//End for (i=cbLessKindItem-1;i>0;i--)
				if (i==0) break;
			}
	...
	}//End if (cbKindItemCount>=cbLessKindItem)
...
bool CGameLogic::IsCanTingCard(BYTE cbCardIndex[MAX_INDEX], tagWeaveItem WeaveItem[], BYTE cbWeaveCount)
{
	...
	CTime tmCur = CTime::GetCurrentTime();
	CString strTime = tmCur.Format("%m%d");
	strFile.Format("log\\%sIsCanTingCard.log",strTime);

	strTemp.Format("IsCanTingCard cbWeaveCount=%d", cbWeaveCount);
	theApp.WriteLog(strFile, strTemp);
...
//	if ((cbCardCount<2)||(cbCardCount>MAX_COUNT)||((cbCardCount-2)%3!=0)) 
	if ((cbCardCount<2)||(cbCardCount>MAX_COUNT))
	{
		
		strTemp.Format("return %d", (cbCardCount-2)%3);
		theApp.WriteLog(strFile, strTemp);
