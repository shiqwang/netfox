...
{
	TCHAR szCurrencyDB[] = TEXT( "KYGameDB" );
	//设置属性
	...
	lstrcpyn(m_GameServiceAttrib.szDescription,TEXT("广东麻将游戏服务组件"),CountArray(m_GameServiceAttrib.szDescription));
	lstrcpyn(m_GameServiceAttrib.szServerModuleName,TEXT("GD_MjServer.DLL"),CountArray(m_GameServiceAttrib.szServerModuleName));
	lstrcpyn(m_GameServiceAttrib.szClientModuleName,TEXT("GD_Mj.EXE"),CountArray(m_GameServiceAttrib.szClientModuleName));
...
bool __cdecl CGameServiceManager::RectifyServiceOption(tagGameServiceOption * pGameServiceOption)
{
	...
	{
		pGameServiceOption->lLessScore=__max(pGameServiceOption->lCellScore*1L,pGameServiceOption->lLessScore);
