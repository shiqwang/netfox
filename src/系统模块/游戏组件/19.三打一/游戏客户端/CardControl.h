...
#define DEF_CARD_V_SPACE			19									//默认间距
#define DEF_CARD_H_SPACE			15									//默认间距
...
public:
	...
	DWORD GetCardCount();
	//获取扑克
	DWORD GetShootCard(BYTE cbCardData[], DWORD dwMaxCount);
	//弹起扑克
	DWORD SetShootCard(BYTE cbCardData[], DWORD dwCardCount);
	//设置扑克
	DWORD SetCardData(const BYTE cbCardData[], DWORD dwCardCount);
	//获取扑克
...
//扑克控件
class CUserCardControl : public CWnd
{
	//配置变量
protected:
	bool							m_bDisplay;							//显示标志
...
protected:
	CRgn							m_CardRegion;						//控件区域
	CArrayTemplate<BYTE>			m_CardDataItem;						//扑克数据
