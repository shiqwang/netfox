...
#define IDI_CALL_SCORE					204								//叫分定时器
...
#define TIME_CALL_SCORE					10								//叫分时间
...
CGameClientDlg::CGameClientDlg() : CGameFrameDlg(&m_GameClientView)
{
	...
	m_GameClientView.m_CallScore.m_cbPriScore=250;
	...
	case IDI_LAND_SCORE:		//叫分
		{
			...
			{
				if ((IsLookonMode()==false)&&(wChairID==GetMeChairID())) OnLandScore(250,250);
				return false;
			}
	...
	case GS_UG_SCORE:  //叫分
		{
			...
			m_cbMainValue=pStatusScore->cbMainValue;
			m_wBankerUser=pStatusScore->wBankerUser;

			//叫牌信息
			m_cbCallCard=pStatusScore->cbCallCard;
			m_cbCallCount=pStatusScore->cbCallCount;
...
bool CGameClientDlg::OnSubCallCard(const void * pBuffer, WORD wDataSize)
{
	...
	BYTE cbCallColor=pCallCard->cbCallColor;
	//逻辑组件
...
bool CGameClientDlg::OnSubCallScore(const void * pBuffer, WORD wDataSize)
{
	...
	m_GameClientView.SetLandScore(wViewChairID,pLandScore->bPrevScore);

	/*
	if (pLandScore->bPrevScore==255)
		{
			m_GameClientView.SetPassFlag(pLandScore->bCallScoreUser,true);
		}
	else
		{
			m_GameClientView.SetPassFlag(pLandScore->bCallScoreUser,false);
		}

	玩家设置
	if ((m_GameClientView.m_bPass[pLandScore->wCurrentUser]==false))
	{
	*/
	BYTE bPreScore=pLandScore->bPrevScore;   //上次叫分
	m_GameClientView.m_CallScore.m_cbPriScore=bPreScore;
	...
	//设置时间
	SetGameTimer(pLandScore->wCurrentUser,IDI_LAND_SCORE,20);
...
bool CGameClientDlg::OnSubTurnBalance(const void * pBuffer, WORD wDataSize)
{
	...
	//if ((wWinnerUser!=m_wBankerUser)&&((wWinnerUser+2)%GAME_PLAYER!=m_wBankerUser))  庄家和对家
	if ((wWinnerUser!=m_wBankerUser))
	{
		//获取分牌
		for (WORD i=0;i<GAME_PLAYER;i++)
		{
	...
	case IDI_DISPATCH_CARD:		//派发扑克
		{
			...
			if (m_cbRemnantCardCount==0) 
			{
				...
				if ((IsLookonMode()==false)&&(m_cbFocusUser==GetMeChairID())) 
				{
					//ActiveGameFrame();
					//设置时间
					SetGameTimer(m_cbFocusUser,IDI_LAND_SCORE,15);
					m_GameClientView.m_CallScore.ShowWindow(SW_SHOW);
...
LRESULT CGameClientDlg::OnLandScore(WPARAM wParam, LPARAM lParam)
{
	...
	LandScore.bBossScore=(BYTE)wParam;
