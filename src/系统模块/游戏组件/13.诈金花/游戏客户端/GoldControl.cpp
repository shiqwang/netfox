...
{
	LONG lGold=0L;
	for (LONG i=0;i<CountArray(m_lGoldCell);i++) lGold+=m_lGoldCell[i]*(LONG)pow((float)10L,i);
	return lGold;
}
...
void CGoldControl::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	{
		LONG lAddGold=(LONG)pow((float)10L,iCellPos);
	...
	if(m_lGoldCell[iCellPos]!=0)                               //add
		AfxGetMainWnd()->SendMessage(IDM_ONFOLLOW,0,0);
...
void CGoldControl::OnRButtonUp(UINT nFlags, CPoint point)
{
	...
	{
		LONG lAddGold=9L*(LONG)pow((float)10L,iCellPos);
		if ((GetGold()+lAddGold)>m_lMaxGold) return;
	}
