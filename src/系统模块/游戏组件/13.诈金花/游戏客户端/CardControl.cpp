...
CCardControl::CCardControl()
{
	...
	{
		m_bInit=true;
		m_ImageCard.SetLoadInfo(IDB_CARD2,AfxGetInstanceHandle());
		m_ImageBack.SetLoadInfo(IDB_CARD2,AfxGetInstanceHandle());
	}
...
//扑克位置
WORD CCardControl::GetCardXPos(BYTE bCardData)													   //很关键的地方。取的扑克图片的左上角坐标
{
	BYTE bCardVolue=(bCardData)%13;
	if (bCardVolue==0) return 12*CARD_WIDTH;
	return (bCardVolue-1)*CARD_WIDTH;
}

//扑克位置
WORD CCardControl::GetCardYPos(BYTE bCardData)
{
	return (bCardData-1)/13*CARD_HEIGHT;
}
...
////////////////////////////////////////////////////////////////////////// add
//设置显示
void CCardControl::SetShowCardFlag(bool flag)               
{
	bDrawCard=flag;

	//重画控件
	if (m_hWnd!=NULL) Invalidate(false);                                                           //此句不可缺少，这是关键所在
