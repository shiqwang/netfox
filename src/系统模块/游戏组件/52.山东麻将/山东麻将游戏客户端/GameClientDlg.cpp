...
//	ON_WM_TIMER()
	...
	//ON_MESSAGE( IDM_OUT_TIME, OnPlaySound )
...
bool CGameClientDlg::InitGameFrame()
{
	...
	//设置标题
	SetWindowText(TEXT("茶馆游戏中心 -- 山东麻将"));
...
bool CGameClientDlg::OnTimerMessage(WORD wChairID, UINT nElapse,UINT nTimerID)
{
	//switch(nTimerID)
	//{
	//	case IDM_OUT_TIME:			//用户出牌
	//	{
	//		//超时判断
	//		if (nElapse==0)
	//		{
	//			if ((IsLookonMode()==false)&&(wChairID==GetMeChairID())) PlayGameSound(AfxGetInstanceHandle(),TEXT("OUT_CARD"));
	//			//KillGameTimer(IDI_OUT_TIME);
	//			//MessageBox("jjjjjjjjjjjjjjj");
	//			return false;
	//		}

	//		//播放声音
	//		//if (m_byHandMjCount<m_byOutMj) return true;
	//		if ((nElapse<=10)&&(wChairID==GetMeChairID())&&(IsLookonMode()==false))
	//		{				
	//			PlayGameSound(AfxGetInstanceHandle(),TEXT("GAME_WARN"));
	//			//MessageBox("bbbbbbbbbbb");
	//			return true;
	//		}
...
LRESULT CGameClientDlg::OnHuMj( WPARAM wParam, LPARAM lParam )
{
	...
	PlayGameSound(AfxGetInstanceHandle(),TEXT("hu2"));
...
//	CopyMemory( &m_byHandMj, pSendMj->byMjs, sizeof( pSendMj->byMjs ) );
	...
	//设置时钟
	m_GameClientView.m_Timer.SetTimerUp( m_GameClientView.m_wCurrentUser, 30, true );
	m_GameClientView.UpdateGameView( NULL );
	return true;
}
//出牌
bool CGameClientDlg::OnSubOutMj( const void * pBuffer, WORD wDataSize )
{
	...
	//SetGameTimer(pOutMj->wOutMjUser,150,IDI_OUT_TIME);
...
bool CGameClientDlg::OnSubPengMj( const void *pBuffer, WORD wDataSize )
{
	...
	//消牌处理
	//m_GameClientView.m_Timer.SetTimerUp( m_GameClientView.m_wCurrentUser, 30, true );	
	SetGameTimer(Peng->wActionUser,IDM_PENG_MJ,20);
	//出牌区减一
	...
	const tagUserData *pUserData = GetUserData(Peng->wActionUser);
	...
	{
		PlayGameSound(AfxGetInstanceHandle(),TEXT("peng0"));
...
bool CGameClientDlg::OnSubChiMj( const void *pBuffer, WORD wDataSize )
{
	...
	//消牌处理
	//m_GameClientView.m_Timer.SetTimerUp( m_GameClientView.m_wCurrentUser, 30, true );	
	SetGameTimer(Chi->wActionUser,IDM_CHI_MJ,20);
	//出牌区减一
	...
	const tagUserData *pUserData = GetUserData(Chi->wActionUser);
	if(pUserData->cbGender == 0)
	{
	...
	{
		PlayGameSound(AfxGetInstanceHandle(),TEXT("chi0"));
...
bool CGameClientDlg::OnSubGangMj( const void *pBuffer, WORD wDataSize )
{
	...
	//消牌处理
	//m_GameClientView.m_Timer.SetTimerUp( m_GameClientView.m_wCurrentUser, 30, true );	
	SetGameTimer(Gang->wActionUser,IDM_GANG_MJ,20);
	//出牌区减一,,明杠才减
	if( Gang->gangType==1)
	...
	const tagUserData *pUserData = GetUserData(Gang->wActionUser);
	if(pUserData->cbGender == 0)
	{
		PlayGameSound(AfxGetInstanceHandle(),TEXT("gang"));
	}
	else if(pUserData->cbGender == 1)
		PlayGameSound(AfxGetInstanceHandle(),TEXT("gang2"));
	return true;
}
