...
{
	m_ImageTimeBack.SetLoadInfo(uIdBack,AfxGetInstanceHandle());
	m_ImageArrow.SetLoadInfo(uIdArrow,AfxGetInstanceHandle());
	CImageHandle HandleBack(&m_ImageTimeBack);
	//CImageHandle HandleArrow(&m_ImageArrow);
	m_Size.cx=m_ImageTimeBack.GetWidth();
	m_Size.cy=m_ImageTimeBack.GetHeight();
}
...
{
	CPaintDC dc(this);
	SetupRgn( &dc, m_ImageTimeBack, RGB( 255, 0, 255) );
	//获取位置
	CRect rcClient;
	...
	m_ImageTimeBack.AlphaDrawImage( &BufferDC, 0, 0, COLORTRANS );
	switch(m_dwCurrentIndex)
	{
		case 0:
			m_ImageArrow.AlphaDrawImage(&BufferDC,rcClient.Width()/2-m_ImageArrow.GetWidth()/8,0,16,16,0,0,RGB(255,0,255));
			break;
		case 3:
			if (m_bDes)
...
void CTimerControl::OnTimer(UINT_PTR nIDEvent)
{
	if (m_nTimer!=-1 && m_bPause==FALSE)
	{
		m_nTimer--;
		Invalidate();
		/*if (m_nTimer<=10 && m_bNotify && m_pSinkWindow)
		{
			m_pSinkWindow->SendMessage(IDM_OUT_TIME,0,0);			
		}*/
