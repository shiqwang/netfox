...
protected:
	...
	BYTE							m_bKouPaiPlayerCount;				//扣牌人数
	bool							m_bXianPaiCompetency[GAME_PLAYER];	//掀牌资格
	tagUserOutCardInfo				m_bUserOutCardInfo[GAME_PLAYER];	//用户出牌信息

	//运行信息
protected:
	BYTE							m_bTurnCardCount;					//出牌数目
	BYTE							m_bTurnCardData[16];				//出牌列表
	CArray<WORD,WORD>				m_bZhanPaiData[3];					//点牌列表
	//BYTE							m_bOutCardData[16];					//一轮的出牌列表
