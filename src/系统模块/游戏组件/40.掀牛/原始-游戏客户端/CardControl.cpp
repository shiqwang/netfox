...
//设置扑克图片
void CCardControl::SetCardImage(UINT nResourceID)
{
	m_bImage.LoadBitmap(nResourceID);
	if(m_bLoad)
	{
		m_ImageCard.RemoveLoadInfo();
		m_bLoad=false;
	}
		//资源变量
	if (m_bLoad==false)
	{
		//加载资源
		m_bLoad=true;
		if (GetSystemMetrics(SM_CXSCREEN)>=1024) m_ImageCard.SetLoadInfo(nResourceID,AfxGetInstanceHandle());
		else m_ImageCard.SetLoadInfo(IDB_CARD_800,AfxGetInstanceHandle());
...
void CCardControl::OnPaint()
{
	...
	//SetupRgn(&dc,m_bImage,RGB(255,0,255));
	...
	for (INT i=0;i<m_CardDataItem.GetCount();i++)
	{
		...
		//m_ImageCard.AlphaDrawImage(&BufferDC,dwXScreenPos,dwYScreenPos,RGB(255,0,255));
	}
...
//////////////////////////////////////////////////////////////////////////
void CCardControl::SetupRgn(CDC *pDC,				//窗体的DC指针
