...
{
	LONG lGold=0L;
//Doctor	for (LONG i=0;i<CountArray(m_lGoldCell);i++) lGold+=m_lGoldCell[i]*(LONG) pow(10L,i);
	return lGold;
}
...
void CGoldControl::OnPaint()
{
	...
	m_ImageLeft.AlphaDrawImage(&BackFaceDC,0,0,RGB(255,0,255));
	...
	m_ImageRight.AlphaDrawImage(&BackFaceDC,ClientRect.Width()-m_ImageRight.GetWidth(),0,RGB(255,0,255));
	...
	BackFaceDC.SelectObject(&CSkinAttribute::m_DefaultFont);
...
void CGoldControl::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	{
		LONG lAddGold=0; ////Doctor(LONG)::pow(10L,iCellPos);
...
void CGoldControl::OnRButtonUp(UINT nFlags, CPoint point)
{
	...
	{
		LONG lAddGold;////Doctor=9L*(LONG)::pow(10L,iCellPos);
		if ((GetGold()+lAddGold)>m_lMaxGold) return;
	}
