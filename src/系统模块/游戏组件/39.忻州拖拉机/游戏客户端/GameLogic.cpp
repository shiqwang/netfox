...
bool CGameLogic::EfficacyOutCard(const BYTE cbOutCardData[], BYTE cbOutCardCount, const BYTE cbFirstCardData[], BYTE cbFirstCardCount, const BYTE cbHandCardData[], BYTE cbHandCardCount)
{
	...
	if ( /*Del by 20080325 (cbFirstCardType!=CT_THROW_CARD)&&*/(cbOutLogicColor==cbFirstLogicColor)&&(cbOutCardType==cbFirstCardType)) return true;
...
//////////////////////////////////////////////////////////////////////////
	//获得牌面值
CString CGameLogic::GetCardStr(BYTE cbCardData)
{
CString	resultStr = "未知";	

CString cardColorStr;
CString cardNumStr;

CString cardColorStrList[4] = {"方块","梅花","红桃","黑桃"};
CString cardNumStrList[4] = {"J","Q","K","A"};
...
//获得花色
cardColorStr.Format("%s", cardColorStrList[GetCardColor(cbCardData)>>4] );
//获得牌点

int cardInt = GetCardValue(cbCardData);

if (cardInt == 1)
{
cardNumStr.Format("%s", cardNumStrList[3]);
}
else if ( cardInt>10 )
{
cardNumStr.Format("%s", cardNumStrList[cardInt-11]);
}
else
{
cardNumStr.Format("%d", cardInt);
}

resultStr.Format("%s %s", cardColorStr, cardNumStr);
}
return resultStr;
}