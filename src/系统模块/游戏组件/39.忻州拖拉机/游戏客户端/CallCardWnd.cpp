...
//输入：掩码， 大王数，小王数
//返回：
void CCallCardWnd::UpdateCallControlXZ(BYTE cbCallColor,int isDKing, int isXKing ,int isNT)
{
	//更新控件
	//只能反不能定
	if ( ( isNT > 0 ) && (isDKing>0)&& (isXKing > 0))
	{
	...
	{
		m_btCallHeiTao.EnableWindow(FALSE);
		m_btCallMeiHua.EnableWindow(FALSE);
	}
	...
	{
		m_btCallHongTao.EnableWindow(FALSE);
		m_btCallFangKuai.EnableWindow(FALSE);
	}
}
//更新控制
