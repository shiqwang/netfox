...
//逆时针
	...
	{//
		...
		m_ptNameNew[3].x=nWidth-m_nXBorder-m_nXFace-10;
		m_ptNameNew[3].x+=xInt;
		m_ptName[3].y=nameStrY;//nViewHeight/2-m_nYFace-23;

		m_ptTimer[3].x=nWidth-m_nXBorder-m_nXTimer*2-5 - VIDEO_PIC_W;
		m_ptTimer[3].x+=xInt;
		m_ptTimer[3].y=nViewHeight/2-m_nYFace-m_nYTimer-30;

		m_ptReady[3].x=nWidth-m_nXBorder-m_nXTimer-5;
		m_ptReady[3].x+=xInt;
		m_ptReady[3].y= m_ptName[3].y;//nViewHeight/2-m_nYFace-m_nYTimer/2-30;

		m_PointBanker[3].x=nWidth-m_nXBorder-m_SizeBanker.cx-3;
		m_PointBanker[3].x+=xInt;
		m_PointBanker[3].y=m_ptName[3].y-bankerUpInt;//nViewHeight/2-20;

		m_ptFace[1].x=m_nXBorder+5;
		m_ptFace[1].y=nViewHeight/2-m_nYFace-25;
		m_ptNameNew[1].x=m_nXBorder+m_nXFace+10;
		m_ptName[1].y=nameStrY;//nViewHeight/2-m_nYFace-23;
		m_ptTimer[1].x=m_nXBorder+5 + VIDEO_PIC_W;
		m_ptTimer[1].y=nViewHeight/2-m_nYFace-m_nYTimer-30;
		m_ptReady[1].x=m_nXBorder+5+m_nXTimer;
		m_ptReady[1].y=m_ptName[1].y;//nViewHeight/2-m_nYFace-m_nYTimer/2-30;
		m_PointBanker[1].x=m_nXBorder+3;
		m_PointBanker[1].y=m_ptName[1].y-bankerUpInt;//nViewHeight/2-20;
	}
//顺时
	else
	{
		...
		m_ptNameNew[1].x=nWidth-m_nXBorder-m_nXFace-10;
		m_ptNameNew[1].x+=xInt;
		m_ptName[1].y=nameStrY;//nViewHeight/2-m_nYFace-23;

		m_ptTimer[1].x=nWidth-m_nXBorder-m_nXTimer*2-5 - VIDEO_PIC_W;
		m_ptTimer[1].x+=xInt;
		m_ptTimer[1].y=nViewHeight/2-m_nYFace-m_nYTimer-30;
		...
		m_PointBanker[1].x+=xInt;
		m_PointBanker[1].y=m_ptName[1].y-bankerUpInt;//nViewHeight/2-20;

		m_ptFace[3].x=m_nXBorder+5;
		m_ptFace[3].y=nViewHeight/2-m_nYFace-25;
		m_ptNameNew[3].x=m_nXBorder+m_nXFace+10;
		m_ptName[3].y=nameStrY;//nViewHeight/2-m_nYFace-23;
		m_ptTimer[3].x=m_nXBorder+5 + VIDEO_PIC_W;
		m_ptTimer[3].y=nViewHeight/2-m_nYFace-m_nYTimer-30;
	...
	m_ptNameNew[0].x=nWidth/2+5+m_nXFace/2;
	m_ptName[0].y= 10;//m_nYBorder+25;

	m_ptTimer[0].x=nWidth/2-m_nXFace/2-m_nXTimer*2-5;
	m_ptTimer[0].y=m_nYBorder+ VIDEO_PIC_H;
	m_ptReady[0].x=nWidth/2-m_nXTimer-m_nXFace/2-5;
	m_ptReady[0].y= m_ptName[0].y;//m_nYBorder+m_nYTimer/2+10;

	m_PointBanker[0].x=nWidth/2-m_nXFace/2-m_SizeBanker.cx-m_nXTimer*2-10;
	m_PointBanker[0].y= 5;//m_nYBorder+3;
	...
	m_ptTimer[2].y=nViewHeight-m_nYTimer-10;
	m_ptTimer[2].y+=yInt;

	m_ptReady[2].x=nWidth/2-m_nXTimer-m_nXFace/2-5;
	m_ptReady[2].y=nViewHeight-m_nYTimer/2-10;
	m_ptReady[2].y	+=yInt;

	m_PointBanker[2].x=nWidth/2-m_nXFace/2-m_SizeBanker.cx-m_nXTimer*2-10;
	m_PointBanker[2].y=nViewHeight-m_SizeBanker.cy-3;
	m_PointBanker[2].y	+=yInt;
	...
	DeferWindowPos(hDwp,m_btStart,NULL,nWidth/2,nViewHeight-rcButton.Height()-50,0,0,uFlags);

	yInt+= 25;
	//留底按钮
	m_btSendConceal.GetWindowRect(&rcButton);
	DeferWindowPos(hDwp,m_btSendConceal,NULL,(nWidth-rcButton.Width())/2,nViewHeight-220+yInt,0,0,uFlags);

	//出牌按钮
	m_btOutCard.GetWindowRect(&rcButton);
	DeferWindowPos(hDwp,m_btOutCard,NULL,nWidth/2+10,nViewHeight-220+yInt,0,0,uFlags);
	DeferWindowPos(hDwp,m_btOutPrompt,NULL,nWidth/2-rcButton.Width()-10,nViewHeight-220+yInt,0,0,uFlags);

	//功能按钮
	m_btConcealCard.GetWindowRect(&rcButton);
	int btnWInt = rcButton.Width();
	int btnHInt = rcButton.Height();

	int btnBlankH = 5;

	int btnXInt = nWidth-m_nXBorder-(btnWInt+5);
	int btnYInt = nViewHeight-5-rcButton.Height();

	DeferWindowPos(hDwp,m_btStusteeControl,NULL, btnXInt, btnYInt,0,0,uFlags);

	btnYInt+=(btnHInt+btnBlankH);
	DeferWindowPos(hDwp,m_btRuquestLeave,NULL,btnXInt, btnYInt,0,0,uFlags);

	btnYInt+=(btnHInt+btnBlankH);
	DeferWindowPos(hDwp,m_btConcealCard,NULL,btnXInt, btnYInt,0,0,uFlags);

	btnYInt+=(btnHInt+btnBlankH);
	DeferWindowPos(hDwp,m_btLastTurnCard,NULL,btnXInt, btnYInt,0,0,uFlags);
	...
	//Doctor m_CardScore.SetBenchmarkPos(m_nXBorder+200,nViewHeight+5,enXLeft,enYTop);
	m_CardScore.SetBenchmarkPos(m_nXBorder+5,m_nYBorder+60,enXLeft,enYTop);
	m_UserCardControl[0].SetBenchmarkPos(nWidth/2,m_nYBorder+m_nYFace+25 + cardYInt2,enXCenter,enYTop);
	m_UserCardControl[2].SetBenchmarkPos(nWidth/2,nViewHeight-198-m_nYBorder+yInt,enXCenter,enYBottom);
	m_HandCardControl.SetBenchmarkPos(nWidth/2,nViewHeight-m_nYFace-m_nYBorder-10+yInt,enXCenter,enYBottom);

	//用户扑克
	if (m_bDeasilOrder==true)
	{
		m_UserCardControl[1].SetBenchmarkPos(nWidth-m_nXFace-m_nXBorder-80 -110,nViewHeight/2-71+cardYInt2,enXRight,enYCenter);
		m_UserCardControl[3].SetBenchmarkPos(m_nXBorder+m_nXFace+80+60,nViewHeight/2-71+cardYInt2 + 60,enXLeft,enYCenter);
	}
	else 
	{
		m_UserCardControl[3].SetBenchmarkPos(nWidth-m_nXFace-m_nXBorder-80 -110,nViewHeight/2-71+cardYInt2,enXRight,enYCenter);
		m_UserCardControl[1].SetBenchmarkPos(m_nXBorder+m_nXFace+80 + 60,nViewHeight/2-71+cardYInt2,enXLeft,enYCenter);
	}

	//叫牌窗口
	CRect rcCallCard;
	m_CallCardWnd.GetWindowRect(&rcCallCard);
	m_CallCardWnd.SetWindowPos(NULL,nWidth/2+20,nViewHeight-m_nYBorder-rcCallCard.Height()-162+yInt,0,0,SWP_NOZORDER|SWP_NOSIZE);

	//积分视图
	CRect rcScore;
	m_ScoreView.GetWindowRect(&rcScore);
	m_ScoreView.SetWindowPos(NULL,(nWidth-rcScore.Width())/2 - 12,nViewHeight/2-160 + 70,0,0,SWP_NOZORDER|SWP_NOSIZE);

	//底牌窗口
	CRect rcConceal;
	m_ConcealCardView.GetWindowRect(&rcConceal);
	m_ConcealCardView.SetWindowPos(NULL,(nWidth-rcConceal.Width())/2,nViewHeight/2-160 + 70,0,0,SWP_NOZORDER|SWP_NOSIZE);
...
void CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	{
//Doctor		m_ImageStationBack.BitBlt(pDC->m_hDC,nXPos,nHeight-m_ImageStationBack.GetHeight()-m_nYBorder);
	}
...
//Doctor		DrawTextString(pDC,szString,RGB(240,240,240),RGB(50,50,50),m_nXBorder+65,nHeight-m_nYBorder-m_SizeStation.cy+14+MOVEPOS);
		cardYInt = m_nYBorder+17;
		DrawTextString(pDC,szString,RGB(240,240,240),RGB(50,50,50),m_nXBorder+70, cardYInt);
	...
	//				nColorImageWidth*(m_cbCallColor>>4),0,RGB(255,0,255));
		...
		//Doctor	DrawTextString(pDC,szString,RGB(240,240,240),RGB(50,50,50),m_nXBorder+85,nHeight-m_nYBorder-m_SizeStation.cy+14+MOVEPOS);
			DrawTextString(pDC,szString,RGB(240,240,240),RGB(50,50,50),m_nXBorder+85,cardYInt);
...
/*	if ((m_cbValueOrder[0]!=VALUE_ERROR)&&(m_cbValueOrder[1]!=VALUE_ERROR))
	...
	if (m_wBankerUser!=INVALID_CHAIR)
	{
		...
		if (pUserData!=NULL)
		{
			//庄家名字
			pDC->SetTextAlign(TA_LEFT|TA_TOP);
		//Doctor	DrawTextString(pDC,pUserData->szName,RGB(240,240,240),RGB(50,50,50),m_nXBorder+65,nHeight-m_nYBorder-m_SizeStation.cy+37+MOVEPOS);
		DrawTextString(pDC,pUserData->szName,RGB(240,240,240),RGB(50,50,50),m_nXBorder+210, cardYInt);
	...
	if (m_wCallCardUser!=INVALID_CHAIR)
	{
		...
		{
			pDC->SetTextAlign(TA_LEFT|TA_TOP);
	//Doctor		DrawTextString(pDC,pUserData->szName,RGB(240,240,240),RGB(50,50,50),m_nXBorder+65,nHeight-m_nYBorder-m_SizeStation.cy+60+MOVEPOS);
	//		cardYInt = m_nYBorder+40;
			DrawTextString(pDC,pUserData->szName,RGB(240,240,240),RGB(50,50,50),m_nXBorder+210, cardYInt);
	...
	//	DrawTextString(pDC,szCardScore,RGB(240,240,240),RGB(50,50,50),m_nXBorder+65,nHeight-m_nYBorder-m_SizeStation.cy+81+MOVEPOS);
		DrawTextString(pDC,szCardScore,RGB(240,240,240),RGB(50,50,50),m_nXBorder+ 70, cardYInt);
...
/*
	...
	*/
	...
	if ( isSysChouInt == 1)
	{
		...
		DrawTextString(pDC,strChouStr,RGB(240,240,240),RGB(50,50,50), (nWidth)/2, (nHeight)/2);
	}
