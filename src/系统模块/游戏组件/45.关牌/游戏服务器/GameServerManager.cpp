...
CGameServiceManager::CGameServiceManager(void)
{
	...
	lstrcpyn(m_GameServiceAttrib.szDescription,TEXT("南通关牌游戏服务组件"),CountArray(m_GameServiceAttrib.szDescription));
	lstrcpyn(m_GameServiceAttrib.szClientModuleName,TEXT("NT_Close.EXE"),CountArray(m_GameServiceAttrib.szClientModuleName));
	lstrcpyn(m_GameServiceAttrib.szServerModuleName,TEXT("NT_CloseServer.DLL"),CountArray(m_GameServiceAttrib.szServerModuleName));
...
bool __cdecl CGameServiceManager::RectifyServiceOption(tagGameServiceOption * pGameServiceOption)
{
	...
	{
		pGameServiceOption->lLessScore=__max(pGameServiceOption->lCellScore*24L,pGameServiceOption->lLessScore);
