...
CTableFrameSink::CTableFrameSink()
{
	...
	//Doctor
	memset(m_bQiangInfo,0,sizeof(m_bQiangInfo));
	m_bQiangLandScore = 0;					//抢地主次数 3
	m_bQiangLand = 1;						//倍数 1 2 4 8
	...
	case GER_NORMAL:		//常规结束
		{
			...
			for (WORD i=0;i<m_wPlayerCount;i++)
			{
				if (i==m_wBankerUser) 
				{
					GameEnd.lGameScore[i]=m_bQiangLand * m_wBombTime*m_bLandScore*lCellScore*((bLandWin==true)?2:-2);
				}
				else 
				{
					GameEnd.lGameScore[i]=m_bQiangLand * m_wBombTime*m_bLandScore*lCellScore*((bLandWin==true)?-1:1);
				}
			}
	...
	case SUB_C_QIANG_LAND://用户抢地主
		{
			...
			//消息处理
			CMD_C_QiangLand * pLandScore=(CMD_C_QiangLand *)pDataBuffer;
			return OnUserQiangLand(pUserData->wChairID,pLandScore->bLandScore);
...
bool __cdecl CTableFrameSink::OnFrameMessage(WORD wSubCmdID, const void * pDataBuffer, WORD wDataSize, IServerUserItem * pIServerUserItem)
{
	...
	//抢地主事件
bool CTableFrameSink::OnUserQiangLand(WORD wChairID, BYTE bLandScore)
{
//效验状态
	if (m_pITableFrame->GetGameStatus()!=GS_WK_QIANG) return true;
	if (wChairID!=m_wCurrentUser) return false;

	//效验参数
	//if (((bLandScore>3)&&(bLandScore!=255))||(bLandScore<=m_bLandScore)) return false;
	if (((bLandScore>3)&&(bLandScore!=255))) return false;
	...
	if (bLandScore!=255)
	{
		m_bQiangInfo[wChairID]=bLandScore;
		m_bQiangLand *= 2;
		m_wBankerUser=m_wCurrentUser;
	}

	//开始判断
	if (m_bQiangLandScore>=3)
	{
		//设置变量
		if (m_bQiangLandScore==0) m_bQiangLandScore=1;
	...
	m_pITableFrame->SendTableData(INVALID_CHAIR,SUB_S_QAING_LAND,&LandScore,sizeof(LandScore));
	m_pITableFrame->SendLookonData(INVALID_CHAIR,SUB_S_QAING_LAND,&LandScore,sizeof(LandScore));
...
bool CTableFrameSink::OnUserLandScore(WORD wChairID, BYTE bLandScore)
{
	...
	{
		//设置状态
		m_pITableFrame->SetGameStatus(GS_WK_QIANG);

		//设置用户
		m_wCurrentUser=(wChairID+1)%m_wPlayerCount;

		//发送消息
		CMD_S_QiangLand GameStart;
		GameStart.bLandUser=m_wBankerUser;
		GameStart.wCurrentUser=m_wCurrentUser ;
		GameStart.bLandScore=m_bLandScore;
		GameStart.bCurrentScore=0;

		m_pITableFrame->SendTableData(INVALID_CHAIR,SUB_S_QAING_LAND,&GameStart,sizeof(GameStart));
		m_pITableFrame->SendLookonData(INVALID_CHAIR,SUB_S_QAING_LAND,&GameStart,sizeof(GameStart));
