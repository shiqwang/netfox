...
class CCardControl : public CWnd
{
	//配置变量
protected:
	bool							m_bDisplay;							//显示标志   m_bShowCard;					//显示扑克
	bool							m_bHorizontal;						//横放标志   m_bHorzLook;					//是否横放
	bool							m_bPositively;						//响应标志   m_bUseMouse;					//响应鼠标

	//间隔变量
protected:
	DWORD							m_dwCardHight;					//扑克高度
	DWORD							m_dwCardWidth;					//扑克宽度
	DWORD							m_dwCardHSpace;						//横向间隔 m_dwCardHSpace;					//横扑克间隔
	DWORD							m_dwCardVSpace;						//竖向间隔 m_iCardVSpace;					//竖扑克间隔
	DWORD							m_dwShootAltitude;					//弹起高度 m_iCardUpSpace;					//跳起高度
