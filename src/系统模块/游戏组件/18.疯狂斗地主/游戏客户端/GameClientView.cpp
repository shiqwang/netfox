...
#define IDC_QIANG					108	
#define IDC_NO_QIANG						109	
//////////////////////////////////////////////////////////////////////////
...
BEGIN_MESSAGE_MAP(CGameClientView, CGameFrameView)
	...
	ON_WM_RBUTTONUP()
	ON_WM_LBUTTONDBLCLK()
	ON_BN_CLICKED(IDC_QIANG, OnQiang)
	ON_BN_CLICKED(IDC_NO_QIANG, OnNoQiang)
...
CGameClientView::CGameClientView(void) : CGameFrameView(true,24)
{
	...
	m_PicBomb.SetLoadInfo(IDB_BITMAP_BOMB,hInstance);;//炸弹
	m_PicPlane.SetLoadInfo(IDB_BITMAP_PLANE,hInstance);;//飞机
	m_PicRocket.SetLoadInfo(IDB_BITMAP_SOCKET,hInstance);;//火箭
...
int CGameClientView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_btQiang.Create(TEXT(""),WS_CHILD,CreateRect,this,IDC_QIANG);
	m_btNoQiang.Create(TEXT(""),WS_CHILD,CreateRect,this,IDC_NO_QIANG);
	...
	m_btQiang.SetButtonImage(IDB_QIANG,hInstance,false);
	m_btNoQiang.SetButtonImage(IDB_NO_QIANG,hInstance,false);
...
void CGameClientView::RectifyGameView(int nWidth, int nHeight)
{
	...
	int btnH = nHeight-195-m_nYBorder-15;
	DeferWindowPos(hDwp,m_btOneScore,NULL,nWidth/2-135-rcButton.Width()/2, btnH,0,0,uFlags);
	DeferWindowPos(hDwp,m_btTwoScore,NULL,nWidth/2-45-rcButton.Width()/2, btnH,0,0,uFlags);
	DeferWindowPos(hDwp,m_btThreeScore,NULL,nWidth/2+45-rcButton.Width()/2, btnH,0,0,uFlags);
	DeferWindowPos(hDwp,m_btGiveUpScore,NULL,nWidth/2+135-rcButton.Width()/2, btnH,0,0,uFlags);

	m_btQiang.GetWindowRect(&rcButton);
	DeferWindowPos(hDwp,m_btQiang,NULL,(nWidth-rcButton.Width())/2-45,nHeight-195-m_nYBorder,0,0,uFlags);
	DeferWindowPos(hDwp,m_btNoQiang,NULL,(nWidth-rcButton.Width())/2+45,nHeight-195-m_nYBorder,0,0,uFlags);
	...
	if (m_bDeasilOrder==true)
	{
		...
		m_LeaveCardControl[0].SetBenchmarkPos(nWidth-m_nXBorder-m_nXFace-50, leaveCardH,enXRight,enYCenter);
		m_LeaveCardControl[1].SetBenchmarkPos(m_nXBorder+m_nXFace+50, leaveCardH,enXLeft,enYCenter);
	}
	else 
	{
		...
		m_LeaveCardControl[1].SetBenchmarkPos(nWidth-m_nXBorder-m_nXFace-50, leaveCardH,enXRight,enYCenter);
		m_LeaveCardControl[0].SetBenchmarkPos(m_nXBorder+m_nXFace+50, leaveCardH,enXLeft,enYCenter);
	}
...
//	m_ImageCenter.BitBlt(pDC->m_hDC,CRect(0,0,nWidth,nHeight),CRect(0,0,m_ImageCenter.GetWidth(),m_ImageCenter.GetHeight()));
	m_ImageCenter.BitBlt(pDC->m_hDC,( nWidth-m_ImageCenter.GetWidth())/2 , 0,SRCCOPY);
	...
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
		...
		if (pUserData!=NULL)
		{
			...
			if (pUserData->cbUserStatus>=US_PLAY)
			{
				...
				pDC->TextOut(m_ptNameNew[i].x,m_ptName[i].y+18,szBuffer,lstrlen(szBuffer));
			}
...
//Add by doctor 
			_snprintf(szBuffer,sizeof(szBuffer),TEXT("倍数：%d "), m_wXNum);
			pDC->TextOut(m_nXBorder+m_nXFace+10,m_nYBorder+42,szBuffer,lstrlen(szBuffer));
	...
	for (int i=0; i < MAX_MOIVE_NUM; i++)
	{
		if(	myMoiveList[i].useStatus == 1)
		{
//	strTemp.Format("into DrawOneMoivePic i=%d", i);
//	WriteLog(strFile, strTemp);

			DrawOneMoivePic(  pDC,  myMoiveList[i]);
		}
	}//End for
...
void CGameClientView::SetCardCount(WORD wChairID, BYTE bCardCount)
{
	...
	//倍数
void CGameClientView::SetXNum(WORD wXNum)
{
	//设置变量
	m_wXNum = wXNum;
...
void CGameClientView::SetLandUser(WORD wChairID, BYTE bLandScore)
{
	...
	//抢地主分数
void CGameClientView::SetQiangLand(WORD wChairID, BYTE bLandScore)
{
	//设置变量
	if (wChairID!=INVALID_CHAIR) m_bQiangLand[wChairID]=bLandScore;
	else memset(m_bQiangLand,0,sizeof(m_bQiangLand));
...
//抢地主按钮
void CGameClientView::OnQiang()
{
	AfxGetMainWnd()->SendMessage(IDM_QIANG_LAND,1,1);
	return;
}

//不抢地主按钮
void CGameClientView::OnNoQiang()
{
	AfxGetMainWnd()->SendMessage(IDM_QIANG_LAND,255,255);
	return;
}
...
//画不同的
void CGameClientView::DrawOneMoivePic(CDC * pDC, int iXPos, int iYPos,int currentNum, int opTypeInt)
{
	CString strFile,strTemp;
	CTime tmCur = CTime::GetCurrentTime();
	CString strTime = tmCur.Format("%m%d");
	strFile.Format("log\\%sDrawOneMoivePic.log",strTime);

	strTemp.Format("DrawOneMoivePic(%d,%d, %d, %d",  iXPos,  iYPos, currentNum,  opTypeInt);
	WriteLog(strFile, strTemp);

/*
CGameImageHelper * myImageHandle;
CGameImageHelper ImageHandle0(&m_PicBomb);
CGameImageHelper ImageHandle1(&m_PicPlane);
CGameImageHelper ImageHandle2(&m_PicRocket);
*/
CImageHandle * myImageHandle;
CSkinImage * myPicPtr;
CImageHandle ImageHandle0(&m_PicBomb);
CImageHandle ImageHandle1(&m_PicPlane);
CImageHandle ImageHandle2(&m_PicRocket);

int nX0=iXPos;
int nY0=iYPos;
/*
#define PS_BX_DICE						315											//骰子坐标
#define PS_BY_DICE						125											//骰子坐标
int nX0=PS_BX_DICE;
int nY0=PS_BY_DICE;
*/
int nW0=161;
...
case 2:
 ...
 nW0=107;
 nH0=290;
		myImageHandle = &ImageHandle2;
		myPicPtr = &m_PicRocket;
	}
	break;
}

int nX1=nW0*currentNum;
int nY1=0;
...
*/
myPicPtr->AlphaDrawImage( pDC, nX0, nY0, nW0, nH0, nX1, 0,  RGB(255,0,255));
//m_PicBomb.AlphaDrawImage( pDC, nX0, nY0, nW0, nH0, nX1, 0,  RGB(255,0,255));
	strTemp.Format("AlphaDrawImage(%d,%d, %d, %d, %d",  nX0, nY0, nW0, nH0, nX1);
	WriteLog(strFile, strTemp);

/*
DrawOneTransPic2( pDC,
					  	 nX0, nY0,
						 nW0, nH0,
						 nX1, nY1,
						 nW1, nH1,
...
//画不同的
void CGameClientView::DrawOneMoivePic(CDC * pDC, MOVE_ITEM	myMoiveItem)
{
	int iXPos=myMoiveItem.myPoint.x ;
	int iYPos=myMoiveItem.myPoint.y;
	int currentNum=myMoiveItem.currentInt ;
	int opTypeInt=myMoiveItem.typeInt ;
DrawOneMoivePic(  pDC,  iXPos,  iYPos, currentNum,  opTypeInt);
