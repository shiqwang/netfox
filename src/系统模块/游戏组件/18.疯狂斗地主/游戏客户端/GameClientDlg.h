...
private:
	...
	//播放叫分声音
	//输入：1，2，3，NO
	void PlayJiaoFenSound(int sexInt,int fenInt);

	//播放OutCard声音
	//输入:牌类型
	void PlayCardMovie(int cardTypeInt);

	//播放OutCard声音
	//输入:牌类型
	void PlayOutCardSound(int sexInt, int cardTypeInt, int cardInt = 0);

	//播放PASS声音
	//输入
	void PlayPassSound (int sexInt);
...
protected:
	...
	//用户抢地主
	bool OnSubQiangLand(const void * pBuffer, WORD wDataSize);
...
protected:
	...
	//抢地主消息
	LRESULT OnQiang(WPARAM wParam, LPARAM lParam);
	//不抢地主消息
	LRESULT OnNoQiang(WPARAM wParam, LPARAM lParam);
