...
//中间牌，操作，杠牌
//杠牌结果
void CControlWnd::SetControlInfo(BYTE cbCenterCard, BYTE cbActionMask, tagGangCardResult & GangCardResult ,bool tingBool )
{
	CString strFile,strTemp;
	CTime tmCur = CTime::GetCurrentTime();
	CString strTime = tmCur.Format("%m%d");
	strFile.Format("log\\%sSetControlInfo.log",strTime);


	strTemp.Format("SetControlInfo cbCenterCard=%d, BYTE cbActionMask=%d , cbCardCount=%d, %d,%d,%d,%d", cbCenterCard,  cbActionMask,
		GangCardResult.cbCardCount,
		GangCardResult.cbCardData[0],
		GangCardResult.cbCardData[1],
		GangCardResult.cbCardData[2],
		GangCardResult.cbCardData[3]
		);
	...
	strTemp.Format("如果已经听了 cbCenterCard=%d, BYTE cbActionMask=%d , cbCardCount=%d, %d,%d,%d,%d", cbCenterCard,  cbActionMask,
		GangCardResult.cbCardCount,
		GangCardResult.cbCardData[0],
		GangCardResult.cbCardData[1],
		GangCardResult.cbCardData[2],
	...
	WriteLog(strFile, strTemp);
		...
		for (BYTE i=0;i<CountArray(cbItemKind);i++) if ((m_cbActionMask&cbItemKind[i])!=0) m_cbItemCount++;
	...
	for (BYTE i=0;i<CountArray(cbItemKind);i++)
	{
		if ((m_cbActionMask&cbItemKind[i])!=0)
		{
			...
			for (BYTE j=0;j<3;j++)
			{
				...
				if (i<CountArray(cbExcursion)) cbCardData=m_cbCenterCard+j-cbExcursion[i];
			...
			if ((m_cbActionMask&cbItemKind[i])&WIK_PENG) nXImagePos=nItemWidth;
	...
	for (BYTE i=0;i<CountArray(m_cbGangCard);i++)
	{
		if (m_cbGangCard[i]!=0)
		{
			...
			for (BYTE j=0;j<4;j++)
			{
				...
				g_CardResource.m_ImageTableBottom.DrawCardItem(&BufferDC,m_cbGangCard[i],j*26+12,nYPos+ITEM_HEIGHT+5);
			}
